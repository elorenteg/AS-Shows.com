...
public class CtrlSeientEnRepresentacio implements ICtrlSeientEnRepresentacio {
	...
	public SeientEnRepresentacio getSeientEnRepresentacio(int fila, int columna, String nomL, String sessio) throws CDSeientEnRepresentacioNoExisteix {
		...
		try {
			...
			List<Object> listObj = session.createSQLQuery(sql).setParameter("nomL", nomL).setParameter("sessio", sessio).setParameter("fila", fila).setParameter("columna", columna)
					.setResultTransformer(Criteria.ALIAS_TO_ENTITY_MAP).list();
			
			if (listObj.size() == 1) {
				seient = (SeientEnRepresentacio) session.get(SeientEnRepresentacio.class, new SeientEnRepresentacioPK(fila, columna, nomL, sessio));
			} else 
	...
	public List<SeientEnRepresentacio> getAllSeientEnRepresentacio() {
		...
		try {
			...
			for (Object obj : listObj) {
				...
				SeientEnRepresentacio seient = (SeientEnRepresentacio) session.get(SeientEnRepresentacio.class, new SeientEnRepresentacioPK(fila, columna, nomL, sessio));
